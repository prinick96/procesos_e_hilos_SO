...
int main() {
    ...
    // Mostrar promedios
    cout << "\nPromedio total: " << (promd_h1 + promd_h2 + promd_h3 + promd_h4 ) / 100 << endl;
    ...
    for(int i = p->rango[0]; i <= p->rango[1]; i++) {
        switch(p->hilo_actual) {
            case 1:
                promd_h1 += arreglo[i];
                if(p->rango[1] == i) {
                    cout << "\nPromedio HILO 1: " << promd_h1 / 25 << endl;
                    usleep(1000000);
                }
            break;
            case 2:
                promd_h2 += arreglo[i];
                if(p->rango[1] == i) {
                    cout << "\nPromedio HILO 2: " << promd_h2 / 25 << endl;
                    usleep(1000000);
                }
            break;
            case 3:
                promd_h3 += arreglo[i];
                if(p->rango[1] == i) {
                    cout << "\nPromedio HILO 3: " << promd_h3 / 25 << endl;
                    usleep(1000000);
                }
            break;
            case 4:
                promd_h4 += arreglo[i];
                if(p->rango[1] == i) {
                    cout << "\nPromedio HILO 4: " << promd_h4 / 25 << endl;
                    usleep(1000000);
                }
            break;
        }
